...
def code_controller(argosfile, controller):
	...
	if controller == "buzz":
		...
		par_def = "\t\t\t<params alpha='" + str(ALPHA_VAL) + "' delta='" + str(DELTA_VAL) + "' velocity='" + str(VELOCITY_VAL) + "' />\n"
...
def code_boundary(argosfile):
	...
	code_box(argosfile, "wall_east", [0.1, ARENA_SIZE, 1], [(ARENA_SIZE/2.0) - 0.05, 0, 0, 0, 0, 0], move=False)
	code_box(argosfile, "wall_west", [0.1, ARENA_SIZE, 1], [-(ARENA_SIZE/2.0) + 0.05, 0, 0, 0, 0, 0], move=False)
	code_box(argosfile, "wall_south", [ARENA_SIZE - 0.2, 0.1, 1], [0, -(ARENA_SIZE/2.0) + 0.05, 0, 0, 0, 0], move=False)
	code_box(argosfile, "wall_north", [ARENA_SIZE - 0.2, 0.1, 1], [0, (ARENA_SIZE/2.0) - 0.05, 0, 0, 0, 0], move=False)
...
# Function writes code for a box entity
def code_cyl(argosfile, cyl_id, height, radius, pose, move=False, mass=1.0):
	...
	cyl_init = "\n\t\t<cylinder id='" + cyl_id + "' height='" + str(height) + "' radius='" + str(radius) + "' "
	cyl_move = "movable='" + moveTag + "' mass='" + str(mass) + "'>\n"
	cyl_pos = "\t\t\t<body position='" + str(pose[0]) + ", " + str(pose[1]) + ", " + str(pose[2]) + "' "
	cyl_ori = "orientation='" + str(pose[3]) + ", " + str(pose[4]) + ", " + str(pose[5]) + "' />\n"
	cyl_end = "\t\t</cylinder>\n"
	cyl_def = cyl_init + cyl_move + cyl_pos + cyl_ori + cyl_end

	argosfile.write(cyl_def)
...
def code_obstacle(argosfile):
	...
	code_cyl(argosfile, "cyl", 0.01, 0.01, [4.5, 4.5, 0, 0, 0, 0], move=False)
